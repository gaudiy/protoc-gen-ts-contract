...
- `pnpm add -D @winor30/protoc-gen-ts-contract`
- Build before generation: `pnpm -C packages/ts-contract-plugin build`
...
plugins:
  - remote: buf.build/bufbuild/es:v2.10.1
    ...
    opt:
      - target=ts
  - local: ./packages/ts-contract-plugin/dist/cli.js
    out: ./gen
    opt:
      - target=ts          # required
      # - skip_pkg=google.*  # override defaults
```
Steps: `pnpm install` → `pnpm -C packages/ts-contract-plugin build` → `pnpm example:gen` (writes protobuf-es + contracts under `example/gen/`).
...
## Output example
Generated `fixtures/orders/v1/order.contract.ts`:
```ts
// @generated by protoc-gen-ts-contract
import type { MoneyInput } from "../../common/v1/common.contract";
